...
try {
  ...
  // description of a small tile is not evidence.
  {
    ...
    // lines and read as broken. A stack under the dropdown is invisible to every other
    // check, so the geometry is measured here, where every run passes, not only the shot
    // runs at the width the store listing uses.
    await look.evaluate("document.getElementById('tabCleaning').click()");
    await sleep(300);
    const addRow = await look.evaluate(`(() => {
      const r = (id) => {
        const el = document.getElementById(id);
        if (!el) return null;
        const b = el.getBoundingClientRect();
