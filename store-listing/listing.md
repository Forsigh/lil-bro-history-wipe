...
```
Keeps the user's rules, settings and an optional local log of wiped entries in browser storage. Settings and the log stay on the device. The rule list is held in local extension storage with everything else, so nothing the user types into this extension is placed anywhere the browser would upload: no account, no server, and no transfer between devices, whether or not browser sync is switched on. Session storage holds the site of an open tab so a closed tab's cookies can be cleared; it is memory, cleared when the browser closes.
```
