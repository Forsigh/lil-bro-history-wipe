...
```
Lil Bro cleans the things you would rather not have on your machine, and it does it without
you thinking about it.

You give it a list: a site, a word, the beginning of an address. Anything on that list is
what it goes after. A bookmark to a shop you are embarrassed about, a forum thread, a name
you typed once. Add it from the popup with one click, or right-click a page and pick it from
the menu.
...
difference: cookies are cleared for the whole site, so you will be signed out of that site
afterwards. Keep list included, for the sites you want to stay logged in on.

What it cannot do:

- Passwords are not touched. The browser removed that for extensions, so there is no switch for it.
- The browser cannot narrow history, download records or form text to one site, so those go for
  the span you choose, not per site.
- The browser will not let anything run at the exact moment it closes, so a clean set for
  close happens at your next start instead. Ninety-nine times out of a hundred you will not
  notice.
